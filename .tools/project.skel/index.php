...
catch(FileException $fse){
	Single::def('HuLOG')->toLog('Error in file/net operations: ' . $fse->getMessage(), 'ERR', 'file/net');
}
catch(VariableRequiredException $vre){
	Single::def('HuLOG')->toLog('Error. Variable required: ' . $vre->varName(), 'ERR', 'var', new OutExtraDataBacktrace($vre->bt));
}
catch(DatabaseConnectErrorException $dbec){//In other DB exception handler used Single::def(__db)->getError() which may prodyce cycle

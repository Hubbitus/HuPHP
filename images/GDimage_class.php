...
//		throw new Exception ('test qwerty');
//		return false;
		}
	return $this->imgINIT();
	}#m getFile
...
//		if (call_user_func($func,$this->img,$srcobj->getResource(),0,0,0,0,$dst_width,$dst_height,$this->PARAM['w'],$this->PARAM['h'])){
...
//	$this->resize($this,0,0,0,0,$dst_width,$dst_height,$this->PARAM['w'],$this->PARAM['h']);
...
/*
		...
		case 9:
		$col = str_split($color, 3);
		break;#9

		default:
		$this->error[] = 'ERROR: Цвет задан не корректно!';
		return false;
		}

		if ($col[0] <= 255 and $col[1] <= 255 and $col[2] <= 255){
		return imagecolorallocate($this->img, $col[0], $col[1], $col[2]);
		}
		else{
		echo $this->error[] = 'ERROR: Цвет задан некорректно!';
		return false;
		}
*/
	}#m parseUserColor
...
//		throw ImageException ('WARNING: Картинка больше заданного, не увеличиваем.');
		...
		if ($newHeight['do']){#Ширину рассчитываем, высота остается как есть
			...
			case 'bottom':
			#$newHeight['topImg'] = $newWidth['full'] - $this->PARAM['w'];
			$newHeight['topImg'] = $newHeight['full'] - $this->PARAM['h'];
			break;#bottom
			}
		}
		else $newHeight['topImg'] = 0;
//		imagealphablending($npic,0);#Отключение прозрачности вообще

...
//Переопределяем!
//ТЕПЕРЬ MAXXOFFRIGHT будет указывать правую гранизу вмещающего блока,
//ТОЕСТЬ ОТ ШИРИНЫ ЗАВИСИТ сколько элемент сможет двигаться!
dd.maxOffX = function(d_x, d_y){
return(
	(
	dd.obj.maxoffl+1 && (d_y = dd.obj.defx-dd.obj.maxoffl)-d_x > 0
		||
	dd.obj.maxoffr+1 && (d_y = dd.obj.defx+dd.obj.maxoffr-dd.obj.w)-d_x < 0
	)
	? d_y
	: d_x
	);
...
//Переопределяем!
//ТЕПЕРЬ MAXXOFFRIGHT будет указывать нижнюю границу вмещающего блока,
//ТОЕСТЬ ОТ ВЫСОТЫ ЗАВИСИТ сколько элемент сможет двигаться!
dd.maxOffY = function(d_x, d_y){
return(
	(
	dd.obj.maxofft+1 && (d_y = dd.obj.defy-dd.obj.maxofft)-d_x > 0
		||
	dd.obj.maxoffb+1 && (d_y = dd.obj.defy+dd.obj.maxoffb-dd.obj.h)-d_x < 0)
	? d_y
	: d_x
	);
};
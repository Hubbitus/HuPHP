...
* Also yu may want define AUTO_INCLUDE_ADDON_FILE, and it is also will be included.
* WARNING: It must itself proper merge/replace/add values into main array, it just included after main!
**/
if ( !defined('AUTO_INCLUDE_FILE') ) define('AUTO_INCLUDE_FILE', '__autoload.map.php');
...
include_once('System/OS.php');//It must be explicit yet
	...
	return true;
		...
		if (!is_set('__autoload_map', @$GLOBALS['__CONFIG'])){
			if( OS::is_includeable(AUTO_INCLUDE_FILE) ){
			require_once(AUTO_INCLUDE_FILE);//Standard
				if ( AUTOLOAD_DEBUG ) fprintf(STDERR, 'include(AUTO_INCLUDE_FILE) [%s]' . "\n", AUTO_INCLUDE_FILE);
				if ( defined('AUTO_INCLUDE_ADDON_FILE') ){
				require_once(AUTO_INCLUDE_ADDON_FILE);
					if ( AUTOLOAD_DEBUG ) fprintf(STDERR, 'include(AUTO_INCLUDE_ADDON_FILE) [%s]' . "\n", AUTO_INCLUDE_ADDON_FILE);
				}
			}
		}
...
*
* PHP completely do not support autoloading of functions, only classes.
* So, we may do that DIRTY HACK:
* Instead of just using function like MY_COOL_FUNCTION() precedes it by singleton:
* m()->MY_COOL_FUNCTION().
* In PHP >= 5.3.0 (When introduced magic method __callStatic) we may use more
...
* So, safely used it may only with functions who not use references in parameters!!!

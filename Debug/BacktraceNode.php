...
class BacktraceNode implements \Iterator {
	...
	* @param	$N		Number of node, got separately (may be already in $arr).
	...
	**/
	public static function create(?array $arr = null, $N = false){
		/*
	...
	public function formatArgs(?array $format = null, ?int $outType = null): string {
		...
		if ($format === null) {
			$format = $this->_format[$outType]['argtypes']
				...
				?? $GLOBALS['__CONFIG']['backtrace::printout']['FORMAT_CONSOLE']['argtypes']
				?? null;
		}

		if ($format === null) {
			throw new VariableRequiredException(new Backtrace(), 'format', 'No format provided for formatArgs');
		}

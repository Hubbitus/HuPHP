...
include_once('Debug/DumpUtils.php');
...
*
* We MUST use direct array_key_exists instead of isset (or even is_set) check because, according
...
if (!array_key_exists('parseCallParam', $GLOBALS['__CONFIG']['debug'])){
	...
	* is equivalent to
	* dump::c($ttt, '$ttt')
	* This future is very useful, but require Tokenizer class and got time overhead.
...
class Dump extends DumpUtils{
	...
	* @return &mixed $var
	**/
	public static function getHeader(&$header, &$var){
		if ($header){
			return $header;
		}
		elseif(
	...
	**/
	public static function c($var, $header = false, $return = false){
		$ret = '';
	...
	* @return string|void	Depend of parameter $return
	**/
	public static function log($var, $header = false, $return = true){
		$ret = '';
		if ($header) { //As is and only explicitly given, without any magic
			$ret .= $header .':';
		}
		$ret .= DumpUtils::transformCorrect_print_r(print_r($var, true))."\n";
		if ($return){
			return $ret;
		}
		else {
	...
	* @return string|void	Depend of parameter $return
	**/
	public static function buff($var, $header = false, $debug_func = 'print_r', mixed $extra){
		/*
		* For use with family ob_*!
		* In this case do not restricted use standard print_r, var_dump and var_export
		*
		...
		*/
		...
		$cmd = 'echo "Throws '.$print_func.'(unserialize('.addcslashes(escapeshellarg(serialize($var)),'"').')'.($extra ? ",'".addcslashes($header, '$')."'" : '').');?>" | php';
	...
	**/
	public static function b_c($var, $header = false){
		$header = self::getHeader($header, $var);

		return self::buff($var, $header, 'dump::c');
	}#m b_c
	...
	* @return string|void	Depend of parameter $return
	**/
	public static function w($var, $header = false, $return = false){
		$ret = '';
	...
	* @return string|void	Depend of parameter $return
	**/
	public static function wap($var, $header = false, $return = false){
		$ret = '';
	...
	*
	* @return mixed	::c or ::w invoke with same parameters.
	**/
	public static function auto($var, $header = false, $return = false){
		/**
	...
	* @return mixed	::c() or ::w() invoke with same parameters.
	**/
	public static function a($var, $header = false, $return = false){
	...
	* @return	void
	**/
	public static function e($var, $header = false){
